...
<?php
	...
	$conn = db_connect();	
	  $query1 = sprintf("select pick_up_price from tbl_load_info where load_id = %s and deleteflag = 0 order by pick_up_price",
	   GetSQLValueString($_REQUEST["load-id"], "int"));
	$n = db_select_query($conn, $query1, $rs_price);
	$sum = $rs_price[0]["pick_up_price"];	
	$commission  = $sum * 0.12;
	
	 $query2= sprintf("select sup.load_id, sup.emp_id, sup.truck_id, sup.pick_up_price, sup.pick_up_location,sup.delivery_location,sup.other_Notes,sup.createdate,
	...
	."where sup.load_id = %s and sup.deleteflag = 0 order by tbl_employee_info.first_name",
	 GetSQLValueString($_REQUEST["load-id"], "int"));
...
<main class="pl-1 pt-1">
	<div class="container">	
			...
			<table id="invoice-item-table" class="table table-bordered">
                ...
                <td colspan="2">
                    ...
                    <tr>
                      ...
                      <td><input type="text" name="gross_pay_total" value="<?php echo $rs_price[0]["pick_up_price"]; ?>" id="gross_pay_total1" data-srno="1"  readonly class="form-control input-sm gross_pay_total" /></td>
			...
			<!--Grid column-->
				...
				for($i=0; $i<$n3; $i++){
				?>
				<option value="<?php echo $rs_vehicle[$i]["plat_number"]; ?>"<?php if($rs_vehicle[$i]["vehicle_id"] == $rs_load[0]["truck_id"]) echo " selected"; ?>><?php echo $rs_vehicle[$i]["plat_number"]; ?></option>
