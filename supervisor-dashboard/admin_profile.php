...
<?php
	...
	$conn = db_connect();	
      $query1 = sprintf("select ai.supervisor_id, ai.first_name, ai.last_name,ai.email, ai.mobile,ai.address, tble_countries.country_name "
	."from tbl_supervisor_info ai "
	."left join tble_countries on ai.country_id = tble_countries.country_id "
	."where ai.supervisor_id = %s  and ai.deleteflag = 0 order by ai.first_name, tble_countries.country_name",
	 GetSQLValueString($_SESSION["supervisor_id"], "int"));
			...
			<tbody>
				<?php
					...
					<tr>	
						...
						<td><a href="administrator-update-form.php?supervisor-id=<?php echo $rs_admin[$i]["supervisor_id"]; ?>" class="btn btn-sm" role="button"><i class="fas fa-edit"></i></a></td>	
						...
						<!-- Delete trigger modal-->
							...
							<a href="#" data-href="admin_info_delete.php?supervisor_id-id=<?php echo $rs_admin[$i]["supervisor_id"]; ?>" data-toggle="modal" data-target="#confirm-delete" class="btn btn-sm" role="button"><i class="fas fa-trash-alt"></i></a>  
