...
<?php
	...
	$conn = db_connect();	
	 ...
	 $query1 = sprintf("select  * from tbl_weeks_info  where week = %s order by week ",
	...
	."where sup.load_id = %s and sup.deleteflag = 0 order by tbl_employee_info.first_name",
	 GetSQLValueString($_REQUEST["load-id"], "int"));
	  ...
	  $query3 = sprintf("select emp_id, first_name, last_name from tbl_employee_info where emp_id = %s order by emp_id  ",
