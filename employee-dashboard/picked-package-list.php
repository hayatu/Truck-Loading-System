...
<?php
	...
	pack.current_loc,pack.destination,pack.status_id,pack.zone_name, pack.commnts, pack.city_name, pack.c_first_name,pack.c_last_name,pack.c_mobile, pack.createdate, pack.pickupprice, 
	...
	."where tbl_employee_info.emp_id = %s and  pack.deleteflag = 0 and pickedby = 1 and tbl_status_info.status_id <>2  order by pack.weight ASC",
	 GetSQLValueString($_SESSION["emp_id"], "int"));	  
	$n = db_select_query($conn, $query1, $rs_package);	
  
				$total_packages =0;
				 for($i=0; $i<$n; $i++){
					$total_packages += $rs_package[$i]["pickupprice"] ;
				 }
	db_close($conn);	
?>
...
<main class="pl-1 pt-1">
	<div class="container">
		...
		<div class="row">
           ...
           <div class="col-md-6">
			<span class="btn btn-primary btn-block font-weight-bold" role="button"> Totol Fees: $<?php echo  $total_packages; ?></span>				
			</div>				
		</div>
			<thead>
				<tr>
			...
			<tbody>
				<?php
					...
					<tr>	
						...
						<td><a href="picked-package-update-form.php?package-id=<?php echo $rs_package[$i]["package_id"]; ?>" class="btn btn-sm" role="button"><i class="fas fa-edit"></i></a></td>
						<td>
