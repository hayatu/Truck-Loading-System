...
<?php
	...
	$n1 = db_select_query($conn, $query1, $rs_week);
	 ...
	 $query2 = sprintf("select emp_id, first_name, last_name from tbl_employee_info where emp_id = %s  order by emp_id ",
	 GetSQLValueString($_SESSION["emp_id"], "int"));
	$n2 = db_select_query($conn, $query2, $rs_employee);
	
      $query3= sprintf("select sup.emp_id,
	...
	."where sup.emp_id = %s and sup.deleteflag = 0 order by tbl_employee_info.first_name",
	 GetSQLValueString($_SESSION["emp_id"], "int"));
		$n3 = db_select_query($conn, $query3, $rs_load);
	...
	<form name="form1" action="tarack-weekpay.php" method="POST" enctype="multipart/form-data">
	 ...
	 <!--Grid row-->
			...
			<select name="emp_id" class=" form-control">
				...
				for($i=0; $i<$n2; $i++){
				?>
			<option value="<?php echo $rs_employee[$i]["emp_id"]; ?>"<?php if($rs_employee[$i]["emp_id"] == $rs_load[0]["emp_id"]) echo " selected"; ?> readonly><?php echo $rs_employee[$i]["first_name"].' '.$rs_employee[$i]["last_name"].' '; ?></option>
