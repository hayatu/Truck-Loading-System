...
if(isset($_POST["From"], $_POST["to"])){
	   ...
	   $query1 = sprintf("select *, SUM(pick_up_price) as pick_up_price_total,
	 tbl_employee_info.first_name, tbl_employee_info.last_name "
	."from tbl_load_info "
	."left join tbl_employee_info on tbl_load_info.emp_id = tbl_employee_info.emp_id "	
	."where tbl_load_info.emp_id = %s and week_name = %s and sdate BETWEEN '".$_POST["From"]."' AND '".$_POST["to"]."' and tbl_load_info.deleteflag = 0" ,
	GetSQLValueString($_REQUEST["emp_id"], "int"),
	GetSQLValueString($_REQUEST["week"], "text"));

	$n = db_select_query($conn, $query1, $rs_paycheck);
	
	$sum = $rs_paycheck[0]["pick_up_price_total"];	
	$commission  = $sum * 0.12;
...
<main class="pl-1 pt-1">
	<div class="container">	
              ...
              <!-- Register form -->
              <form name="form1" method="post" action="paycheck-insert.php" enctype="multipart/form-data">
		
                <p class="h5 text-center mb-0 p-2">Paycheck Form </p><!--<p>All fields indicated in (*) are obligatory.</p>-->
			...
			<table id="invoice-item-table" class="table table-bordered">
                ...
                <td colspan="2">
                    ...
                    <tr>
                      ...
                      <td><input type="text" name="gross_pay_total" value="<?php echo $rs_paycheck[0]["pick_up_price_total"]; ?>" id="gross_pay_total1" data-srno="1" readonly class="form-control input-sm gross_pay_total" /></td>
					  <td><input type="text" name="commission" value="<?php echo $commission; ?>" id="commission1" data-srno="1" class="form-control input-sm number_only commission" /></td>
					  <td><input type="text" name="fuel" id="fuel1" data-srno="1" class="form-control input-sm number_only commission" required /></td>
                      <td><input type="text" name="insurance_fee" id="insurance_fee1" data-srno="1" class="form-control input-sm number_only insurance_fee" /></td>
                      ...
                      <td><input type="text" name="other_charges" id="other_charges1" data-srno="1" class="form-control input-sm number_only other_charges" /></td>
					  <td><input type="text" name="net_pay" id="net_pay1" data-srno="1" readonly class="form-control input-sm net_pay" /></td>
                    </tr>
                  </table>	
				<!--<div align="right">
			...
			<!--Grid column-->
				...
				for($i=0; $i<$n3; $i++){
				?>
				<option value="<?php echo $rs_vehicle[$i]["plat_number"]; ?>"<?php if($rs_vehicle[$i]["vehicle_id"] == $rs_paycheck[0]["truck_id"]) echo " selected"; ?>><?php echo $rs_vehicle[$i]["plat_number"]; ?></option>
				...
				<div class="col-md-4 mb-4">
					<div>
						...
						<input name="sdate" type="text" value="<?php echo $_POST["From"]; ?>" id="datepicker_start" placeholder="Start Date" class="form-control" required>  
					</div>
				</div>	
              <!--Grid column-->	
               <!--Grid column-->
				<div class="col-md-4 mb-4">
					<div>
						...
						<input name="edate" type="text" value="<?php echo $_POST["to"]; ?>"  id="datepicker_end" placeholder="End Date" class="form-control" required>  
					</div>
				</div>	
              <!--Grid column-->
                <!--Grid column-->
              ...
              <!--Grid column-->
                <div class="col-md-4 mb-4">
                    <div>
                        ...
                        <textarea name="other_Notes" type="text" class="form-control md-textarea" id="other_Notes" placeholder="Other Notes" rows="3"></textarea>
                    </div>
                </div>
              <!--Grid column-->				
            </div>
		<!--Grid row-->	
               <div class="text-center mt-4">
                 <input name="submit" type="submit" value="Save" id="submit" class="btn btn-primary"/>
